 ...
 * ``Action Object``.*(Optional)* The object representing the action itself.
 * ``Target``. *(Optional)* The object that the verb is enacted on.
 
``Actor``, ``Action Object`` and ``Target`` are ``GenericForeignKeys`` to any arbitrary Django object. An action is a description of an action(``Action Object``) that was performed (``Verb``) at some instant in time by some ``Actor``, with some optional ``Target``.
